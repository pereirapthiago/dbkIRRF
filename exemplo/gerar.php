...
$declaracao->header = new RegistroHeaderDTO(
    ...
    reciboDeclaracaoAnterior: substr($elab->numero_recibo_anterior ?? '', 0, 10),
...
$declaracao->dadosPessoais = new RegistroDadosPessoaisDTO(
    ...
    uf: !empty($endereco->uf) ? UnidadeFederativa::tryFrom($endereco->uf) : null,
    email: $info->email ?? '',
    dddFixo: str_pad((string)((int)($info->ddd ?? 0)), 2, '0', STR_PAD_LEFT),
    telefoneFixo: $info->telefone ?? '',
    ...
    flagResidenciaPais: !empty($info->residente_exterior) ? '1' : '0',
...
foreach ($elab->elab_dependentes ?? [] as $dep) {
    $codigoParentesco = str_pad((string)((int)($dep->grau_parentesco->descricao ?? 0)), 2, '0', STR_PAD_LEFT);
    $declaracao->adicionarDependente(new RegistroDependenteDTO(
        cpf: $cpf,
        sequencial: $seqDependente,
        tipoDependente: $codigoParentesco,
